...
class QNAP_Main_Controller {
    ...
    private function activate_actions() {
        ...
        // Handle AJAX test restore
        add_action( 'wp_ajax_qnap_test_restore', array( 'qnap\QNAP_Status_Controller', 'test_restore' ) );
    }
    ...
    public function qnap_commands() {
        ...
        // Add multisite export support
        if (is_multisite()) {
            add_filter( 'qnap_export', array( 'qnap\QNAP_Export_Multisite', 'execute' ), 7 );
        }
        ...
        // Add multisite import support
        if (is_multisite()) {
            add_filter( 'qnap_import', array( 'qnap\QNAP_Import_Multisite', 'execute' ), 120 );
        }
    ...
    public function qnap_loaded() {
        // For non-multisite or multisite with our built-in support
        if ( ! defined( 'QNAPME_PLUGIN_NAME' ) ) {
            if ( is_multisite() ) {
                // We've added built-in multisite support
    ...
    public function multisite_notice() {
        // We've added built-in multisite support
        QNAP_Template::render('main/multisite-supported');
    }
    ...
    public function enqueue_export_scripts_and_styles( $hook ) {
        if ( stripos( 'toplevel_page_qnap_export', $hook ) === false && stripos( 'qnap-backup_page_qnap_create_export', $hook ) === false ) {
            return;
        }
