...
class QNAP_Import_Content {
    ...
    public static function execute( $params ) {
        ...
        // Set progress
        $message = sprintf( __( 'Restoring files...', QNAP_PLUGIN_NAME ));
        $params = QNAP_Progress_Bar::update($params, $processed_files_size, $total_files_size, $message);
        ...
        while ( $archive->has_not_reached_eof() ) {
            ...
            if ($processed_files_size % (5 * 1024 * 1024) < $file_bytes_written || $files_processed % 10 == 0) {
                $message = sprintf( __( 'Restoring files...', QNAP_PLUGIN_NAME ));
                $params = QNAP_Progress_Bar::update($params, $processed_files_size, $total_files_size, $message);
            }
        ...
        if ( $archive->has_reached_eof() ) {
            ...
            // Set progress to 100%
            $message = sprintf( __( 'Finished restoring %d files.', QNAP_PLUGIN_NAME ), $total_files_count);
            $params = QNAP_Progress_Bar::update($params, $total_files_size, $total_files_size, $message);
